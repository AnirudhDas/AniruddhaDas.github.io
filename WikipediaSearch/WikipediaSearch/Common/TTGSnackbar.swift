...
/*
 Used to customize the Snackbar view.
 */
class Snackbar {
    
    static func configure(message: String, duration:TTGSnackbarDuration, icon: UIImage?, backgroundColor: UIColor, textColor: UIColor) -> TTGSnackbar {
        ...
        // Custom font
        if let font = UIFont(name: "OpenSans-Regular", size: 13) {
        ...
        //To show snackbar above the safe area margin for iPhone X
        if UIDevice.current.screenType == .iPhoneX {
            snackbar.bottomMargin = 40.0 //Height of bottom safe area margin
        }
...
open class TTGSnackbar: UIView {
    ...
    @objc open dynamic var contentInset: UIEdgeInsets = UIEdgeInsets.init(top: 6, left: 13, bottom: 6, right: 10) {
        didSet {
...
public extension TTGSnackbar {
    ...
    public func show() {
        ...
        dismissTimer = Timer(timeInterval: (TimeInterval)(duration.rawValue), target: self, selector: #selector(dismiss), userInfo: nil, repeats: false)
